...
import { Users, GraduationCap, BookOpen, Bell, Calendar, TrendingUp, UserCheck, Clock } from "lucide-react"
    ...
    <div className="space-y-8">
              ...
              return (
                <div key={activity.id} className="flex items-start space-x-4">
                  <div className={`rounded-full p-2 ${activity.type === "success" ? "bg-green-100" : "bg-blue-100"}`}>
                    <Icon className={`h-4 w-4 ${activity.type === "success" ? "text-green-600" : "text-blue-600"}`} />
                  </div>
            ...
            {quickActions.map((action) => {
              ...
              return (
                <Link key={action.title} to={action.href}>
                  <div className={`rounded-lg border p-4 transition-colors hover:bg-accent ${action.color}`}>
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Current class assignments and teacher allocations</CardDescription>
        </CardHeader>
        <CardContent>
          <div className="grid gap-4 md:grid-cols-2 lg:grid-cols-3">
            {[
              { class: "5A", teacher: "Sarah Johnson", students: 28, subject: "Mathematics" },
              { class: "6B", teacher: "Emily Davis", students: 25, subject: "Science" },
              { class: "4C", teacher: "Lisa Anderson", students: 30, subject: "Art" },
              { class: "3A", teacher: "Not Assigned", students: 22, subject: "-" },
              { class: "2B", teacher: "Not Assigned", students: 26, subject: "-" },
              { class: "1A", teacher: "Not Assigned", students: 24, subject: "-" },
            ].map((classInfo) => (
              <div key={classInfo.class} className="rounded-lg border p-4">
                <div className="flex items-center justify-between">
                  <h3 className="font-semibold">Class {classInfo.class}</h3>
                  <Badge variant={classInfo.teacher === "Not Assigned" ? "secondary" : "default"}>
                ...
                <div className="mt-2 space-y-1">
                  ...
                  {classInfo.subject !== "-" && (
                    <p className="text-sm text-muted-foreground">Subject: {classInfo.subject}</p>
                  )}
                </div>

...
const TeacherDashboardPage = () => {
  ...
  const quickActions = [
    { title: "Create Assignment", icon: FileText, color: "bg-blue-500", path: "/teacher/assignments/new" },
    { title: "Schedule Test", icon: Calendar, color: "bg-green-500", path: "/teacher/tests/new" },
    { title: "Grade Papers", icon: CheckCircle, color: "bg-orange-500", path: "/teacher/grading" },
    { title: "View Analytics", icon: TrendingUp, color: "bg-purple-500", path: "/teacher/analytics" }
  ];
            ...
            <CardContent>
              <div className="space-y-4">
                {recentClasses.map((class_) => (
                  <div key={class_.id} className="flex items-center justify-between p-4 border rounded-lg hover:bg-gray-50">
                    <div className="flex items-center gap-3">
                      <div className={`w-3 h-3 rounded-full ${
                        class_.status === 'ongoing' ? 'bg-green-500' : 
                        class_.status === 'upcoming' ? 'bg-blue-500' : 'bg-gray-400'
                      }`} />
                      <div>
              ...
              <div className="space-y-4">
                {topPerformers.map((student) => (
                  <div key={student.name} className="flex items-center justify-between p-3 bg-gray-50 rounded-lg">
                    ...
                    <div className="text-right">
                      <div className="font-bold text-green-600">{student.grade}%</div>
                    ...
                    <div>
                      ...
                      <div className="flex items-center gap-4 mt-2">
                        <div className="text-xs">
                          <Progress value={(item.submissions / item.total) * 100} className="w-20 h-2" />
                          <span className="text-muted-foreground">{item.submissions}/{item.total} submitted</span>
                        </div>
                      </div>
                    </div>
                  </div>
                  <div className="text-right">
                    <Button size="sm" className="mb-2">Grade Now</Button>
